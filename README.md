...
- Structured Generation: Gemini response schema와 Pydantic v2 검증으로 추천/채팅/사진 분석 계약을 강제
- Verified Citations: 모델의 `S1`/`S2` 선택을 실제 retrieval 결과와 대조해 검증된 출처만 응답
- Fail-closed Coaching: 추천/채팅에 신뢰 가능한 RAG 근거가 없으면 무근거 생성을 차단
- Reproducible Validation: API, Gemini, RAG trace, 개인정보, cleanup을 run ID 기준으로 검증
...
AI 요청, atomic daily quota, 실제 Gemini 호출의 lifecycle/attempt 감사 기준은
`docs/AI_REQUEST_LIFECYCLE.md`를 기준으로 한다.
RAG 검색 trace의 개인정보 최소화와 retention 기준은 `docs/RAG_TRACE_PRIVACY.md`를 기준으로 한다.
UI/API/AI 통합 검증과 실패 후 cleanup 기준은 `docs/AI_INTEGRATION_VALIDATION.md`를 기준으로 한다.
...
- `docs/UI_AI_INTEGRATION_VALIDATION_REPORT.md`
- `docs/AI_INTEGRATION_VALIDATION.md`
- `docs/UI_MOBILE_ACCEPTANCE_REPORT.md`

AI 통합 검증:

```bash
docker compose exec -e DEBUG=false backend python -m app.cli.ai validate-integration --report-path /workspace/docs/UI_AI_INTEGRATION_VALIDATION_REPORT.md
...
docker compose exec backend python -m app.cli.rag replacement-evaluate --candidate-id <candidate_id> --report-path /tmp/RAG_REPLACEMENT_CANDIDATE_EVALUATION.md
docker compose exec backend python -m app.cli.rag catalog-replace-source --file rag_sources/catalog.json --key <catalog_key> --replacement-url <url> --activate --evaluation-id <evaluation_id>
