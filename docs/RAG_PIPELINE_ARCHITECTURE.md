...
| Field | Description |
|-------|-------------|
| `origin_type` | `internal_seed`, `file_markdown`, `file_text`, `file_pdf`, `url_html`, `url_pdf`, `api`, `image`, `video` |
...
```text
catalog row
 -> source adapter (`url_html`, `pdf_url`, or `local_file`)
 -> ParsedDocument
 -> ChunkPlan
...
For `pdf_url`, official PDF bytes are fetched directly and fingerprinted with HTTP metadata, content length, and raw hash. The parsed document uses `pdf_text` and `page_paragraph` chunking. Empty/scanned PDFs or files beyond `RAG_URL_MAX_BYTES` are not OCRed in v1; they become manual-review acquisition failures.
