...
`catalog-apply --run-id <id> --review-run-id <review_id>`는 저장된 plan item을 source별로 적용한다.
...
`ingest-catalog`는 bootstrap/direct ingest 용도로 유지한다. 운영 기본 경로는 `catalog-plan` -> `catalog-review` -> `catalog-apply --review-run-id`다.
...
Only after review should an operator execute `catalog-apply --run-id <catalog_plan_run_id> --review-run-id <catalog_review_run_id>`.

---

## 11. Apply Approval Gate

`catalog-apply` now enforces the review layer:
