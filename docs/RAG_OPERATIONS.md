...
## 8. URL Source Acquisition

v1 외부 수집은 crawler가 아니라 운영자가 등록한 공식 URL 단일 페이지 HTML fetch다. 링크를 따라가지 않고, source registry에 명시된 URL만 가져온다.

초기 공식 catalog는 `backend/rag_sources/catalog.json`에 둔다.
...
docker compose exec backend python -m app.cli.rag register-url --url <official-url> --category nutrition --source-grade A
docker compose exec backend python -m app.cli.rag ingest-catalog --file rag_sources/catalog.json
```

URL fetch metadata는 `rag_sources.metadata.fetch_metadata`와 source 컬럼에 남긴다.

| Field | Purpose |
|-------|---------|
| `final_url` | redirect 이후 실제 수집 URL |
| `content_type` | HTML/text 여부와 parser 선택 근거 |
| `etag` | 원격 변경 감지 신호 |
| `last_modified` | 원격 freshness 신호 |
| `fetched_at` | 수집 시각 |
| `raw_content_hash` | 원문 byte 기준 hash |
| `normalized_content_hash` | parser/normalizer 이후 문서 hash |

등록 정책:

- source grade `A`는 공공기관/공식 가이드라인 계열 URL에만 부여한다.
- 동일 `origin_uri` 또는 `final_url`의 active source가 있으면 신규 중복 등록 대신 refresh 경로로 처리한다.
- parser confidence가 기준 미만이면 source를 만들지 않고 `manual_review_required` decision으로 남긴다.
...
URL/HTML source는 Hybrid Chunking을 기본값으로 사용한다.

```text
Document
  -> Parent Section (h1~h3 heading path)
  -> Child Evidence Chunk (paragraph/list/table row group)
```

저장 방식:

- `rag_sections` 테이블은 v1에서 만들지 않는다.
- `rag_chunks.metadata.parent_heading_path`에 section lineage를 저장한다.
- `rag_chunks.metadata.parent_section_hash`는 section 단위 변경 감지에 사용한다.
- `rag_chunks.metadata.chunk_role=child_evidence`로 prompt에 들어갈 근거 chunk임을 표시한다.
- 길이 제한 분할은 의미 단위 chunk가 너무 길 때만 guardrail로 적용한다.
